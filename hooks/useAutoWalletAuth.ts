...
export function useAutoWalletAuth() {
  ...
  useEffect(() => {
    ...
    const authenticateWallet = async () => {
      ...
      try {
        if (process.env.NODE_ENV === "development") console.log('[Auto Wallet Auth] Starting authentication');

        // Get referral code if present
        const referralCode = getReferralCode();
        if (referralCode) {
          if (process.env.NODE_ENV === "development") console.log('[Auto Wallet Auth] Found referral code');
        }
        ...
        if (process.env.NODE_ENV === "development") console.log('[Auto Wallet Auth] Requesting signature');

        // Request signature from wallet
        const signature = await signMessage(encodedMessage);
        const signatureBase58 = bs58.encode(signature);

        if (process.env.NODE_ENV === "development") console.log('[Auto Wallet Auth] Authenticating...');
        ...
        if (result?.error) {
          ...
          if (process.env.NODE_ENV === "development") console.log('[Auto Wallet Auth] Success');
          lastAuthenticatedWallet.current = publicKey.toBase58();
        }
      } catch (error: any) {
        ...
        if (!error.message?.includes('User rejected') && !error.message?.includes('rejected')) {
          ...
          if (process.env.NODE_ENV === "development") console.log('[Auto Wallet Auth] User rejected signature');
        }
      } finally {
