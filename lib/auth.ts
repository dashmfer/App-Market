  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const sanitizedReferralCode = credentials.referralCode && /^[a-z0-9]{4,20}$/.test(credentials.referralCode)
      ...
      async authorize(credentials) {
        ...
        const user = await prisma.$transaction(async (tx) => {
          ...
          if (!txUser) {
            ...
            const validReferralCode = credentials.referralCode && /^[a-z0-9]{4,20}$/.test(credentials.referralCode.toLowerCase())
